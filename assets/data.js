const objectData = [
  {
    id: "1",
    title: "Fenty",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/fenty-1.webp",
    secondImage: "https://vimcosmo.com/pics/fenty-2.webp",
  },
  {
    id: "2",
    title: "Ilia",
    description: "",
    price: 38.65,
    image1: "https://vimcosmo.com/pics/ilia-1.webp",
    secondImage: "https://vimcosmo.com/pics/ilia-2.webp",
  },
  {
    id: "3",
    title: "Kenzoki hydration",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/kenzoki-1.webp",
    secondImage: "https://vimcosmo.com/pics/kenzoki-2.webp",
  },
  {
    id: "4",
    title: "Pixi",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "5",
    title: "Pixi",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "6",
    title: "Pixi",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "7",
    title: "Eyeliner",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "8",
    title: "Crazy Brush",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "9",
    title: "Crazy Brush",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-1.webp",
    secondImage: "https://vimcosmo.com/pics/pixi-2.webp",
  },
  {
    id: "10",
    title: "Lipstick",
    description:
      "A creamy, long-lasting lipstick that provides intense color and hydration.",
    price: 12.99,
    image1: "https://vimcosmo.com/pics/kenzoki-2.webp",
  },
  {
    id: "11",
    title: "Lockoti Eyeliner",
    description:
      "Replenish your skin before bed, no matter what time of day, with this luxurious hydrating gel cream, specially designed by Rihanna for her evening beauty routine. - The rich and light gel cream moisturizes the skin immediately and for 8 hours. - Our ultra-hydrating cream harnesses the benefits of Hyaluronic Acid and the intensely hydrating properties of Kalahari Melon Oil and Baobab to lock in moisture and restore elasticity to the skin.- Wrinkles and fine lines fade instantly and over time",
    price: 23.95,
    image1: "https://vimcosmo.com/pics/pixi-3.webp",
  },
  {
    id: "12",
    title: "Mascara",
    description:
      "A volumizing mascara that lengthens and defines lashes for a dramatic look.",
    price: 14.99,
    image1: "https://vimcosmo.com/pics/fenty-2.webp",
  },
  {
    id: "13",
    title: "Lip Gloss",
    description:
      "A lightweight lip gloss that provides intense color and a glossy finish.",
    price: 11.99,
    image1: "https://vimcosmo.com/pics/fenty-2.webp",
  },
];

const divideProducts = (size) => {
  let productList = [];

  for (let i = 0; i < objectData.length; i += size) {
    console.log(objectData.length);
    productList.push(objectData.slice(i, (i += size)));
  }

  return productList;
};

const dataShowMore = {
  products: divideProducts(4),
  current: 0,
  limit: divideProducts(4).length,
};
